...
class ImageViewer:
    ...
    def gesture_recog(self):
        ...
        def print_result(result: self.GestureRecognizerResult, output_image: mp.Image, timestamp_ms: int):
            if result and result.gestures:
                ...
                ges = self.gesture_control(top_gesture.category_name)
                if ges in ['PalmOpened', 'PalmClosed', 'thumbs_up_cropped']:
                    time.sleep(3)  # Sleep for 3 seconds before recognizing the next gesture

        options = self.GestureRecognizerOptions(
            base_options=self.BaseOptions(model_asset_path='My_model_new1.task'),min_hand_detection_confidence=0.8, 
    ...
    def gesture_control(self, gesture):
        if gesture == 'PalmOpened':
            self.reset()
            return True  # Signal that the gesture was recognized
